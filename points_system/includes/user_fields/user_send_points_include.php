...
if ($profile_method == "input") {
    ...
    $this->lookup = filter_input(INPUT_GET, 'lookup', FILTER_VALIDATE_INT);
    $this->send_point = filter_input(INPUT_POST, 'send_point', FILTER_DEFAULT);
    $userid = fusion_get_userdata('user_id');
    if (iMEMBER && !empty($this->send_point) && $userid != $this->lookup) {
        ...
        if (\defender::safe() && $error == '') {
        	...
        	\PHPFusion\Points\UserPoint::getInstance()->setPoint($this->lookup, ["mod" => 1, "point" => $sendpoints, "messages" => $locale['uf_sendpoints_007']]);
        	$messages = sprintf($locale['uf_sendpoints_009'], fusion_get_userdata('user_name'));
	    	send_pm($this->lookup, $userid, $locale['uf_sendpoints_008'], $messages, $smileys = "y");
	    	addNotice('success', $locale['uf_sendpoints_010']);
        } else {
	    	addNotice('danger', $error);
        }
    }
    if (iMEMBER && $userid != $this->lookup) {

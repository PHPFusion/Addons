...
class Core {
    ...
    protected static function set_body_span() {
        if (empty(self::getParam('main_span'))) {
            $full_sum = 12;
            $_right_status = (self::getParam('right') && defined('RIGHT') && (RIGHT or self::getParam('right_pre_content') or self::getParam('right_post_content')));
            $_left_status = (self::getParam('left') && defined('LEFT') && (LEFT or self::getParam('left_pre_content') or self::getParam('left_post_content')));

...
function render_gallery($info) {
    ...
    if (isset($info['item'])) {
        function render_photoAlbum(array $info = []) {
            ...
            echo "</div>\n";
            echo "<div class='overflow-hide album_thumbnail' style='height: ".($gallery_settings['thumb_h'] - 15)."px'>\n";
            echo $info['image'];
            echo "</div>\n";
...
function render_photo($info) {
    ...
    if ($info['photo_allow_comments'] && fusion_get_settings('comments_enabled') == 1) {
        opentable();
            echo '<div class="well">'.$info['photo_show_comments'].'</div>';
        closetable();
    }

    if ($info['photo_allow_ratings'] && fusion_get_settings('ratings_enabled') == 1) {
        opentable();
            echo '<div class="well">'.$info['photo_show_ratings'].'</div>';
        closetable();
    }
